...
process.on('uncaughtException', (e) => { try { require('fs').appendFileSync('/tmp/bc3_fatal.log', 'UNCAUGHT: ' + (e.stack||e) + '\n'); } catch(_){} });
process.on('unhandledRejection', (e) => { try { require('fs').appendFileSync('/tmp/bc3_fatal.log', 'UNHANDLED: ' + (e&&e.stack||e) + '\n'); } catch(_){} });
process.on('exit', (c) => { try { require('fs').appendFileSync('/tmp/bc3_fatal.log', 'EXIT code=' + c + '\n'); } catch(_){} });
const app = express();
