process.on('uncaughtException', (e) => { try { require('fs').appendFileSync('/tmp/bc4_fatal.log', 'UNCAUGHT: ' + (e.stack||e) + '\n'); } catch(_){} });
process.on('unhandledRejection', (e) => { try { require('fs').appendFileSync('/tmp/bc4_fatal.log', 'UNHANDLED: ' + (e&&e.stack||e) + '\n'); } catch(_){} });
process.on('exit', (c) => { try { require('fs').appendFileSync('/tmp/bc4_fatal.log', 'EXIT code=' + c + ' uptime=' + process.uptime() + '\n'); } catch(_){} });
process.on('SIGTERM', () => { try { require('fs').appendFileSync('/tmp/bc4_fatal.log', 'GOT SIGTERM\n'); } catch(_){} process.exit(0); });
